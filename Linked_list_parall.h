#ifndef _LINKED_LIST_PARALL_H_
#define _LINKED_LIST_PARALL_H_

#include "Linked_list.h"
#include "Parall.h"

// специализация лист для Параллелепипедa

template <>
class linked_list<Parall>
{
private:
    Node<Parall> *head; // указатель на первый узел списка
    Node<Parall> *tail; // указатель на последний узел списка

public:
    // конструктор по умолчанию
    linked_list<Parall>() : head(NULL), tail(NULL) {}

    // конструктор копирования
    linked_list<Parall>(const linked_list<Parall> &other) : head(nullptr), tail(nullptr)
    {
        Node<Parall> *current = other.head; // начинаем с головы другого списка
    ...
    linked_list<Parall>(linked_list<Parall> &&other) : head(other.head), tail(other.tail)
    {
    ...
    linked_list<Parall> &operator+(const Node<Parall> &product)
    {
    ...
    void add_product(const Node<Parall> &new_product)
    {
        Node<Parall> *new_node = new Node<Parall>(new_product); // создаем новый узел, используя конструктор node
    ...
    Node<Parall> *insertAtPos(Node<Parall> *head, int pos, Node<Parall> new_data)
    {

        Node<Parall> *new_node = new Node<Parall>(new_data); // создаем новый узел
    ...
    void save_to_file(const std::string &filename)
    {
        ...
        Node<Parall> *current = head; // начинаем с головы списка
        while (current)               // пока не достигнем конца списка
        {
            int x1, y1, z1, x2, y2, z2, x3, y3, z3, x4, y4, z4, x5, y5, z5, x6, y6, z6, x7, y7, z7, x8, y8, z8;
            double v;
            current->tovar.get_product(x1, y1, z1, x2, y2, z2, x3, y3, z3, x4, y4, z4, x5, y5, z5, x6, y6, z6, x7, y7, z7, x8, y8, z8, v); // получаем данные о товаре из текущего узла
            file << x1 << " " << y1 << " " << z1 << " "
                 << x2 << " " << y2 << " " << z2 << " "
                 << x3 << " " << y3 << " " << z3 << " "
                 << x4 << " " << y4 << " " << z4 << " "
                 ...
                 << x6 << " " << y6 << " " << z6 << " "
                 << x7 << " " << y7 << " " << z7 << " "
                 << x8 << " " << y8 << " " << z8 << " " << v << std::endl; // записываем данные в файл
    ...
    void load_from_file(const std::string &filename)
    {
        ...
        while (!file.eof())           // продолжаем до конца файла
        {
            int x1, y1, z1, x2, y2, z2, x3, y3, z3, x4, y4, z4, x5, y5, z5, x6, y6, z6, x7, y7, z7, x8, y8, z8;
            double v;
            file >> x1 >> y1 >> z1 >> x2 >> y2 >> z2 >> x3 >> y3 >> z3 >> x4 >> y4 >> z4 >> x5 >> y5 >> z5 >> x6 >> y6 >> z6 >> x7 >> y7 >> z7 >> x8 >> y8 >> z8 >> v; // читаем данные из файла
            if (file)                                                                                                                                                  // если данные были успешно прочитаны
            {
                Parall new_product(x1, y1, z1, x2, y2, z2, x3, y3, z3, x4, y4, z4, x5, y5, z5, x6, y6, z6, x7, y7, z7, x8, y8, z8); // создаем новый товар
